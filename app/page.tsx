      ...
      <Tabs value={tab} onValueChange={changeTab}>
        <TabsContent value="ask">
          ...
          {result&&<section className="mt-8 space-y-6">
            <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><div className="flex flex-wrap gap-2"><Badge className="bg-[#071a2d]">{result.judgement}</Badge><Badge variant="outline">{result.recurrence}</Badge></div><p className="mt-3 text-sm text-slate-500">Interpreted as <strong>{result.interpretation?.intent}</strong> · {result.interpretation?.concept}</p>
              <div className="mt-6 divide-y divide-slate-100">{result.sections?.map((s:any)=><div key={s.key} className={s.claim_class==="Teacher inference"?"my-4 rounded-2xl border border-violet-200 bg-violet-50 p-5":"py-5"}><div className="flex flex-wrap items-center gap-2"><h3 className="text-lg font-semibold">{s.heading}</h3><Badge variant="outline" className="text-[11px]">{s.claim_class}</Badge></div><p className="mt-2 whitespace-pre-line leading-7 text-slate-800">{s.text}</p></div>)}</div>
            </div>
