...
check('current spreadsheet graph profile has deterministic current-method post-processing',askRoute.includes('Deterministic current-method guardrail for spreadsheet graph questions') && askRoute.includes('Obtain the gradient and y-intercept directly from the coefficients of that equation') && askRoute.includes('do not calculate the fitted-line gradient from two plotted points or read the intercept manually from the graph.'));
...
check('current spreadsheet graph post-processing removes non-current support sections',askRoute.includes('records.every(record => record.layer === "governing" || record.layer === "specimen")'));
